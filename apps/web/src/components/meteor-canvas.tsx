 ...
 *
 * This module is only ever imported by components/meteors.tsx, and only once
 * that wrapper has established that the theme is dark and reduced motion is off
 * — so the environment check that used to live here has moved there, in front of
 * the download rather than behind it. By the time this component exists, it is
 * wanted.
 */
export default function MeteorCanvas() {
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      // ---- meteors ----
      /*
       * DENSITY BREATHES (Phase 10, Task 7).
       *
       * This used to top the population straight back up to `maxMeteors` every
       * single frame, which meant the sky held a mathematically constant number
       * of streaks for as long as you looked at it. That is the "repetitive"
       * complaint: not that any one meteor was wrong, but that the shower had no
       * weather — no surge, no lull, nothing to notice changing.
       *
       * The target is now modulated by two sine waves whose periods share no
       * common multiple (23s and 61s), so the combined envelope does not repeat
       * on any timescale anyone will sit through. Density ranges from about a
       * third of capacity in a lull to full in a surge. Same particle budget at
       * the peak — the ceiling has not moved, so this costs nothing and mostly
       * draws FEWER meteors than before.
       */
      const ebb =
        0.66 + 0.34 * (Math.sin(now * 0.00027) * 0.6 + Math.sin(now * 0.0001) * 0.4);
      const targetMeteors = Math.max(6, Math.round(maxMeteors * ebb));

      spawnClock -= dt;
      while (spawnClock <= 0 && meteors.length < targetMeteors) {
