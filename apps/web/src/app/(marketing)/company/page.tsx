    ...
    <main className="mx-auto max-w-[1360px] px-6 pb-32 pt-20 md:px-10">
      ...
      {team.length === 0 ? (
        ...
        <Stagger className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {team.map((m) => {
            const { title, specialties } = splitDesignation(m.designation);
            return (
              <Item key={m.id} className="h-full">
                <div className="tilt group relative flex h-full flex-col overflow-hidden rounded-card border border-border-subtle bg-surface-raised text-center">
                  {/* A soft accent bloom over the portrait's lower edge — the region's
                      colour (amber on this inbox-adjacent page) made atmospheric, not a
                      border. It reads as light spilling from the image into the card. */}
                  <span
                    aria-hidden="true"
                    className="pointer-events-none absolute left-1/2 top-[46%] z-10 h-40 w-40 -translate-x-1/2 rounded-full bg-brand-cyan/10 blur-3xl transition-opacity duration-500 group-hover:opacity-80"
                  />
                  {/* The premium portrait: large, face-anchored (source photos put the
                      face in the upper region, so TeamPhoto crops object-top), never
                      stretched. It is the visual focus of the card again. */}
                  <TeamPhoto
                    name={m.name}
                    photo={m.photoUrl}
                    sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw"
                    className="aspect-[4/4.5] w-full"
                    /* Aishika only: pin the crop to the top INLINE so it cannot be
                       overridden by any cached or conflicting utility rule — her
                       portrait's head sits at the very top and must never be cut. */
                    objectPosition={
                      m.photoUrl?.includes("/aishika") ? "center top" : undefined
                    }
                  />
                  <div className="relative z-10 flex flex-1 flex-col items-center p-8 pt-6">
                    <h3 className="text-lg font-semibold tracking-tight">{m.name}</h3>
                    <p className="mt-1 text-sm font-medium text-brand-teal">{title}</p>
                    ...
                    {specialties.length > 0 && (
                      <ul className="mt-3 flex flex-wrap justify-center gap-1.5">
                        {specialties.map((s) => (
                          <li
                            key={s}
                            className="rounded-full border border-border-subtle bg-surface-overlay/60 px-2.5 py-1 text-[12px] leading-none text-text-secondary"
                          >
                            {s}
                          </li>
                        ))}
                    ...
                    {m.links.length > 0 && (
                      <p className="mt-auto flex flex-wrap justify-center gap-x-4 gap-y-1 pt-5">
                        {m.links.map((l) => (
                          <a
                            key={l.label + l.href}
                            href={l.href}
